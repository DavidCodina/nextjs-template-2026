...
function SidebarTrigger({ className, onClick, ...props }: SidebarTriggerProps) {
  const { toggleSidebar, open, isMobile, openMobile, collapsible, side } = useSidebar()
  ...
  return (
    <button
      ...
      className={cn('text-primary hover:bg-primary/15 cursor-pointer rounded-full p-1', className)}
      onClick={(event) => {
        onClick?.(event)
        toggleSidebar()
      }}
      {...props}
    >
      {(open && !isMobile) || (isMobile && openMobile) ? <CloseIcon /> : <OpenIcon />}
