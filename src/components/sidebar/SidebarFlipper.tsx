...
const SidebarFlipper = ({
  ...
  if (side === 'right') {
    return (
      <>
        ...
        <AppSidebar />
        <SidebarTrigger
          className={cn('fixed top-1 right-1 cursor-pointer', SIDEBAR_TRIGGER_ZINDEX_CLASS, sidebarTriggerClassName)}
  ...
  return (
    <>
      ...
      {children}
      <SidebarTrigger
        className={cn('fixed top-1 left-1 cursor-pointer', SIDEBAR_TRIGGER_ZINDEX_CLASS, sidebarTriggerClassName)}
        style={sidebarTriggerStyle}
      />
    </>
  )
