...
export const AlternativeLabelStepSeparator = ({
    ...
    const updateSeparatorWidth = () => {
      ...
      setTimeout(() => {
        if (internalButtonRef.current) {
          ...
          if (gapRef.current) {
            const currentButtonSeparatorSegment = (newButtonWidth - stepCircleSize) / 2
            ...
            const nextButton = stepper?.querySelector(`button[data-index="${currentButtonIndex + 1}"]`)

            if (nextButton && nextButton instanceof HTMLElement) {
              const nextButtonWidth = nextButton.getBoundingClientRect().width
              const nextButtonSeparatorSegment = (nextButtonWidth - stepCircleSize) / 2
  ...
  return (
    <div
      ...
      {...(isValid === false ? { 'data-valid': false } : isValid === true ? { 'data-valid': true } : {})}
      style={{
        position: 'absolute',
        top: '50%',
