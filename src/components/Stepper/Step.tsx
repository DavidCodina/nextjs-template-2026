  ...
  return (
    <>
      ...
      >
        ...
        <StepBody description={description} isCompleted={isCompleted} isValid={isValid} label={label} />
      </button>

      {/* Used to calculate the gapSegment in AlternativeLabelStepSeparator. */}
      {alternativeLabel && !isLast && <div ref={gapRef} className={cn('flex-1 self-center', matches && 'hidden')} />}

      {!alternativeLabel && !isLast && (
        <StepSeparator isCompleted={isCompleted} isValid={isValid} stepCircleRef={stepCircleRef} />
      )}
    </>
  )
}
