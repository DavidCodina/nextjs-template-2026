  ...
  const getShape = (style: string): JSX.Element => {
    switch (shape) {
      ...
      default:
        if (style === 'full') {
          return (
            <FullStar activeColor={activeColor} defaultColor={defaultColor} svgStyle={{ height: size, width: size }} />
          )
        }
        return (
          <EmptyStar activeColor={activeColor} defaultColor={defaultColor} svgStyle={{ height: size, width: size }} />
        )
    }
  }
    ...
    return new Array(count).fill('').map((_, index) => {
      ...
      return (
        ...
        >
          ...
          <div title={showTitle ? titleArray[index] : ''}>{showEmptyIcon ? getShape('empty') : getShape('full')}</div>
        </div>
      )
    })
  }
