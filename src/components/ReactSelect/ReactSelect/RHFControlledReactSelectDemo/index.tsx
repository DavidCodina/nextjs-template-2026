...
import { Button, ReactSelect } from '@/components'
...
export const RHFControlledReactSelectDemo = () => {
  const {
    ...
    handleSubmit,
    control,
    formState: { errors, isValid, touchedFields, isSubmitting, isSubmitted, isSubmitSuccessful }
  } = useForm<FormValues>({
  ...
  useEffect(() => {
    if (isSubmitSuccessful === true) {
      // reset(undefined, {}) // Will reset to defaultValues
      reset({ dessert: null as unknown as FormValues['dessert'] }, {})
  ...
  const renderForm = () => {
    return (
      <Fragment>
        <form
          className='bg-card mx-auto mb-6 max-w-[800px] rounded-lg border p-4 shadow'
          onSubmit={(e) => {
            e.preventDefault()
          }}
            ...
            render={({ field /*, fieldState, formState */ }) => {
              ...
              return (
                <ReactSelect
                  ...
                  // ⚠️ In order for the reset() to work, value={value} needs to be defined.
                  value={value}
                  //# isMulti={true}
  ...
  return (
    <Select
      ...
      noOptionsMessage={(_obj) => {
        ...
        return <div className='text-center font-bold text-blue-500'>Loading...</div>
      }}
      // openMenuOnFocus={true} // default is false
      // openMenuOnClick={false} // default is true
      placeholder='Select Ice Cream...'
