...
jest.mock('../utils', () => ({
  getExchangeRate: jest.fn().mockReturnValue(1.5)
}))
...
describe('getPriceInCurrency()...', () => {
  ...
  it(`should call getExchangeRate() once.`, () => {
    ...
    const getExchangeRateSpy = jest.spyOn(utils, 'getExchangeRate')
    ...
    expect(getExchangeRateSpy).toHaveBeenCalledWith('USD', 'AUD' /* expect.any(String) */)

    // ❌ expect(getExchangeRateSpy).toReturnWith(expect.any(Number))
    expect(getExchangeRateSpy).toHaveReturnedWith(expect.any(Number))
  })
})
