...
describe('toHaveErrorMessage...', () => {
  test('should pass', () => {
    render(
      <>
        <label htmlFor='startTime'> Please enter a start time for the meeting: </label>
        <input
    ...
    // ⚠️ Warning: toHaveErrorMessage has been deprecated and will be removed in future updates. Please use toHaveAccessibleErrorMessage.
    // ❌ expect(timeInput).toHaveErrorMessage('Invalid time: the time must be between 9:00 AM and 5:00 PM')
    expect(timeInput).toHaveAccessibleErrorMessage('Invalid time: the time must be between 9:00 AM and 5:00 PM')

    // ❌ expect(timeInput).toHaveErrorMessage(/invalid time/i) // to partially match
    expect(timeInput).toHaveAccessibleErrorMessage(/invalid time/i) // to partially match

    // ❌ expect(timeInput).toHaveErrorMessage( expect.stringContaining('Invalid time')) // to partially match
    expect(timeInput).toHaveAccessibleErrorMessage(expect.stringContaining('Invalid time')) // to partially match

    // ❌ expect(timeInput).not.toHaveErrorMessage('Pikachu!')
    expect(timeInput).not.toHaveAccessibleErrorMessage('Pikachu!')
  })
})
