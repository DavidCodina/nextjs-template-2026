...
const Amount = () => {
  ...
  return (
    <>
      <input name='amount' onChange={(e) => setValue(e.target.value)} type='number' value={value} />
...
describe('RTL Interactions...', () => {
  ...
  test('should spy on obj.mock and detect being called when clicked.', async () => {
    ...
    const spy = jest.spyOn(obj, 'mockHandler')
  ...
  test('should be be removed on click.', async () => {
    const RemoveOnClick = () => {
      ...
      return (
        <div>
          {show ? <button onClick={() => setShow(false)}>Click Me</button> : <div>The button has been removed.</div>}
        </div>
      )
    }
  ...
  test('should show button shortly after mount, then remove on click.', async () => {
    const ShowOnMount = () => {
      ...
      return <div>{show && <button onClick={() => setShow(false)}>Click Me</button>}</div>
    }
    ...
    const button = await screen.findByRole('button' /* , undefined, { timeout: 1000 } */)
    expect(button).toBeInTheDocument()
