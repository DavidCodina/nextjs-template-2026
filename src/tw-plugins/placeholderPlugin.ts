...
export const placeholderPlugin = plugin(function (pluginApi) {
  ...
  addComponents({
    ...
    '.placeholder-glow': {
      animation: 'glow var(--placeholder-animation-duration, 1.5s) ease-in-out infinite'
    },

    /* With the alpha channel, the closer to 0.0, the brighter the shimmer. */
    '.placeholder-wave': {
      maskImage: 'linear-gradient(135deg, black 33%, rgba(0, 0, 0, 0.5), black 66%)',
    ...
    '.placeholder-white-wave': {
      backgroundImage: 'linear-gradient(135deg, transparent 33%, rgba(255, 255, 255, 0.5), transparent 66%)',
      backgroundSize: '200% 100%',
      animation: 'whiteWave var(--placeholder-animation-duration, 1s) linear infinite'
    },

    '.dark .placeholder-white-wave': {
      backgroundImage: 'linear-gradient(135deg, transparent 33%, rgba(255, 255, 255, 0.25), transparent 66%)'
    },
    ...
    */
    ...
    '.placeholder-shine::after': {
      ...
      background: 'linear-gradient(to bottom right,transparent 40%,rgba(255, 255, 255, 0.3) 50%,transparent 60%)',
